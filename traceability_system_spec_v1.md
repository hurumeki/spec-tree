...
| **フロントエンド** | React + Vite + TypeScript（ブラウザベース SPA） |
| **グラフ描画** | Cytoscape.js |
| **データ交換** | JSONファイル（ファイルシステム経由） |
| **パッケージ管理 / モノレポ** | npm workspaces |
| **テスト** | Vitest |
...
npm workspaces によるモノレポ構成を採用する。ルート直下の `prompts/`, `docs/`, `output/`, `data/` は CLI が読み書きする作業ディレクトリ、`packages/` 配下がアプリケーション本体のワークスペースとなる。

project/

  package.json       \# ルート（workspaces 定義・横断スクリプト）
  ...
  packages/
    ...
    backend/         \# Node.js（TypeScript）REST API サーバ
